...
- `src/app/layout.tsx` — DM Serif Display (`--font-dm-serif`) + Inter (`--font-inter`) from Google Fonts. `SmoothScrollProvider` wraps children. Exports `metadata.icons` with two SVG favicons (`/favicon_lightmode.svg`, `/favicon_darkmode.svg`) gated by `prefers-color-scheme` media queries — Next.js renders them as `<link rel="icon" media="...">` tags. Default `favicon.ico` was removed to avoid duplicate icon emission.
...
**CaseStudy fields:** `slug`, `title`, `subtitle`, `role`, `timeline`, `company`, `category`, `metric`, `heroImage?` (case study page — optional; also acts as the poster for `heroVideo`), `heroVideo?` (autoplay/loop hero video — optional, takes precedence over `heroImage`), `cardImage` (homepage thumbnail — can differ from hero), `overview?`, `sections`. At least one of `heroImage` / `heroVideo` should be set; if neither is, the hero section renders nothing.
...
| `VideoBlock` | `src` (mp4 etc.), `poster?`, `caption?`, `maxWidth?`, `variant?`, `controls?` (default `true`), `autoPlay?` (default `true`), `loop?` (default `true`) — breaks to 1100px. Always `muted + playsInline + preload="metadata"`. Uses the same `ImageFrame` variants as images. |
...
- Hero media renders conditionally: if `cs.heroVideo` is set → `<HeroVideo videoSrc posterSrc={cs.heroImage} maxWidth="800px" />`; else if `cs.heroImage` is set → `<FullBleedImage src={cs.heroImage} variant="borderless" priority />`; else nothing.
- `<OverviewSection overview={cs.overview} />` between hero media and body (role / team / timeline-status chips + overview paragraphs)
- Body sections: `SectionHeading` in 740px column; `FullBleedImage`, `ImageGrid`, `VideoBlock`, `TwoColText`, `TwoColImageText` break out to 1100px; all other blocks stay in 740px column
- Footer: "Next Project →" (cycles through studies) + `<ContactBlock />`

### Case study body components (`src/components/case-study/`)
`SectionHeading`, `SubHeading`, `TextBlock`, `InsightCallout`, `MetricHighlight`, `FullBleedImage`, `HeroVideo`, `VideoBlock`, `ImageGrid`, `PullQuote`, `ListBlock`, `TwoColText`, `TwoColImageText`, `OverviewSection`, `StickyBackButton`, `lib/ImageFrame`.

- `HeroVideo.tsx` — borderless `<video autoPlay muted loop playsInline controls preload="metadata" poster={posterSrc}>` wrapped in the same motion shell as `FullBleedImage`. Used only for the case study hero (driven by `cs.heroVideo`). Defaults to 1100px max width but the page passes `maxWidth="800px"` for the hero.
- `VideoBlock.tsx` — same shape as `HeroVideo` but for inline body content, configurable via the `VideoBlock` content block. Always muted + playsInline; `controls`, `autoPlay`, `loop` default to `true`.
...
**Video assets:** Stored alongside images under each slug folder (e.g. `public/images/case-studies/carloan/hero.mp4`, `DIY-calculator.mp4`). Always re-encode for web before committing — H.264 with `+faststart` (moov atom at the front, otherwise the whole file must download before playback begins), CRF 23, capped around 1600px / 30fps, audio stripped if the video plays muted. Reference command: `ffmpeg -i in.mp4 -vf "scale='min(1600,iw)':-2,fps=30" -c:v libx264 -preset slow -crf 23 -pix_fmt yuv420p -an -movflags +faststart out.mp4`. `<video>` elements bypass `next/image`, so there's no `remotePatterns` requirement.
